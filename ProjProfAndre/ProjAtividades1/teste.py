...
myfunc = pymongo.MongoClient("mongodb://localhost:27017/")
mydb2 = myfunc["Funcionario"]
mycol2 = mydb2["Dados do Funcionario"]

mysale = pymongo.MongoClient("mongodb://localhost:27017/")
mydb3 = mysale["Vendas"]
mycol3 = mydb3["Dados da Venda"]
    ...
    if cadastro == 1:
        ...
        while True:
            ...
            if opcao == 1:
                ...
                for i in range(len(n)):
                    m.append(input(f'{n[i]}: '))
                a = C(f'{m[0]}', f'{m[1]}', f'{m[2]}', f'{m[3]}', f'{m[4]}', f'{m[5]}', f'{m[6]}')
                a.getJson()
                break
            elif opcao == 2:
                arquivo = open("C:/Users/ruiro/Python/ProjPyCharm/ProjAtividades1/DadosFuncionario.csv", 'r')
                linhas = arquivo.readlines()
                lista = dict()
                for l in linhas:
                    i = l.replace('\n', '').split(';')
                ...
                while True:
                    ...
                    if (tipo == 1):
                        ...
                        print('Pesquisa por Salario...')
                        n = str(input(''))
                        for x in mycol.find({"Salario": f"{n}"}):
                            print(x)
                    elif tipo == 8:
                        break
                    else:
        ...
        while True:
            ...
            opcao = int(input('>>> '))
            if opcao == 1:
                print('Dados Para a Venda:')
                x = str(input('Informe o nome do produto que será vendido: '))
                y = str(input('Informe o cliente que está realizando a compra: '))
                z = str(input('Insira sua ID de funcionário: '))
                for p in mycol1.find({"Nome": f'{x}'}):
                    print(p)
                for c in mycol.find({"Nome": f'{y}'}):
                    print(c)
                for f in mycol2.find({"Nome": f'{z}'}):
                    print(f)
            elif opcao == 2:
                print('Dados Para Consulta')
